...
const HERO_COPY = {
  brands: {
    ...
    formLabel: "Set up your store and start selling on YIIVA today.",
    cta: "Start selling",
  ...
  return (
    <section className="hero-backdrop relative px-6 pt-[calc(80px+3.5rem)] pb-14 lg:pt-[calc(80px+6rem)] lg:pb-20 text-center">
      <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center gap-6 lg:gap-8">
        ...
        {/* Primary CTA — Brands: start-selling signup · Shoppers: get the app */}
        {audience === "brands" ? (
          <div className="w-full max-w-md space-y-3">
            <p className="text-[var(--color-ink)] font-medium text-sm">
              {HERO_COPY.brands.formLabel}
            </p>
            ...
            {showSuccess && (
              <div className="bg-[var(--color-paper-2)] border-2 border-[var(--color-ink)] rounded-xl p-4 flex items-start gap-3 text-left">
                ...
                <div>
                  <p className="text-[var(--color-ink)] font-semibold text-sm lg:text-base">You&apos;re all set!</p>
                  <p className="text-[var(--color-ink-60)] text-xs lg:text-sm mt-1">Check your inbox to finish setting up your store.</p>
                </div>
              </div>
            )}
          </div>
